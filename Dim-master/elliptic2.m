...
function res = elliptic1(x_value, xi_value)
    ...
    ay = @(y) 0.1 + exp(0.2*(KL_fun_generator(y, lambda(1), w(1), c, L)*xi(1) + KL_fun_generator(y, lambda(2), w(2), c, L)*xi(2))) + exp(0.1*(KL_fun_generator(y, lambda(4), w(4), c, L)*xi(4) + KL_fun_generator(y, lambda(5), w(5), c, L)*xi(5)));
